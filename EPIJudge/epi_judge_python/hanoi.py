...
def compute_tower_hanoi(num_rings):
    ...
    def move_disks(rings_left, source, target, extra):
        if rings_left > 0:
            move_disks(rings_left - 1, source, extra, target)
            target.append(source.pop())
            move_disks(rings_left - 1, extra, target, source)

    return move_disks(num_rings, [], [], [])
